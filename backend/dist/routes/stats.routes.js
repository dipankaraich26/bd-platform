...
router.get('/dashboard', async (_req, res) => {
    const [totalBusinesses, businessesByStage, businessesBySector, totalProjects, projectsByStatus, recentActivities, upcomingMilestones, avgScore, totalFeedback, feedbackBySentiment,] = await Promise.all([
...
router.get('/pipeline', async (_req, res) => {
    ...
    const data = await Promise.all(stages.map(async (stage) => {
        const [count, value] = await Promise.all([
            prisma.business.count({ where: { stage: stage } }),
